...
internal sealed class NavigationDataE5a : NavigationDataINavFNav
{
	...
	public override byte[] Generate()
	{
		...
		int num = galileoNavicSecondOfWeek / 10;
		int num2 = num % 5;
		int second = num * 10;
		...
		for (int i = 0; i < num3; i++)
		{
			...
			byte[] array = GetPageData(in transmissionTime, pageId, in subframeIndex, ephemeris).ToArray(214);
			IEnumerable<byte> second2 = CRC24Q.ComputeBytes(array);
			IEnumerable<byte> inputSequence = array.Concat(second2).Concat(NavigationDataINavFNav.tailBits);
			satelliteIndex = 0;
			int captureIndex = 0;
	...
	private IEnumerable<byte> GetPageData(in GnssTime pageTime, PageId pageId, in int subframeIndex, Satellite ephemeris)
	{
		...
		case PageId.Page1:
		{
			...
			double value3 = (double)ephemeris.TimeOfApplicability * (1.0 / 60.0);
			bitCount = 14;
			byte[] second37 = NavigationData.Dec2Bin(in value3, in bitCount);
			...
			byte[] second41 = NavigationDataINavFNav.sisaBits;
			IEnumerable<byte> second42 = NavigationData.ZeroBits.Take(5);
			IEnumerable<byte> second43 = NavigationData.ZeroBits.Take(10);
			value = ((!ephemeris.IsHealthy) ? 1u : 0u);
			bitCount = 2;
			byte[] second44 = NavigationData.Dec2Bin(in value, in bitCount);
			bitCount = pageTime.GalileoNavicWeek;
			bitCount2 = 12;
			byte[] second45 = NavigationData.Dec2Bin(in bitCount, in bitCount2);
			bitCount = pageTime.GalileoNavicSecondOfWeek;
			bitCount2 = 20;
			byte[] second46 = NavigationData.Dec2Bin(in bitCount, in bitCount2);
			byte b = (byte)((!ephemeris.IsHealthy) ? 1 : 0);
			byte[] second47 = new byte[1] { b };
			IEnumerable<byte> second48 = NavigationData.OneAndZeroBits.Take(26);
			return first.Concat(second36).Concat(iodNavBits2).Concat(second37)
				.Concat(second38)
				.Concat(second39)
				.Concat(second40)
				.Concat(second41)
		...
		case PageId.Page2:
		{
			...
			double value3 = ephemeris.MeanAnomaly * NavigationDataINavFNav.Rad2Semi * 2147483648.0;
			bitCount = 32;
			byte[] second68 = NavigationData.Dec2Bin(in value3, in bitCount);
			value3 = ephemeris.RateOfLongitudeOfAscendingNode * NavigationDataINavFNav.Rad2Semi * 8796093022208.0;
			bitCount = 24;
			byte[] second69 = NavigationData.Dec2Bin(in value3, in bitCount);
			value3 = ephemeris.Eccentricity * 8589934592.0;
			bitCount = 32;
			byte[] second70 = NavigationData.Dec2Bin(in value3, in bitCount);
			value3 = ephemeris.SqrtA * 524288.0;
			bitCount = 32;
			byte[] second71 = NavigationData.Dec2Bin(in value3, in bitCount);
			value3 = ephemeris.LongitudeOfAscendingNode * NavigationDataINavFNav.Rad2Semi * 2147483648.0;
			bitCount = 32;
			byte[] second72 = NavigationData.Dec2Bin(in value3, in bitCount);
			value3 = ephemeris.RateOfInclination * NavigationDataINavFNav.Rad2Semi * 8796093022208.0;
			bitCount = 14;
			byte[] second73 = NavigationData.Dec2Bin(in value3, in bitCount);
			bitCount = pageTime.GalileoNavicWeek;
			int bitCount2 = 12;
			byte[] second74 = NavigationData.Dec2Bin(in bitCount, in bitCount2);
			bitCount = pageTime.GalileoNavicSecondOfWeek;
			bitCount2 = 20;
			byte[] second75 = NavigationData.Dec2Bin(in bitCount, in bitCount2);
			return first.Concat(iodNavBits3).Concat(second68).Concat(second69)
				.Concat(second70)
				.Concat(second71)
				.Concat(second72)
				.Concat(second73)
		...
		case PageId.Page3:
		{
			...
			byte[] iodNavBits4 = NavigationDataINavFNav.GetIodNavBits(in t0e);
			double value3 = ephemeris.Inclination * NavigationDataINavFNav.Rad2Semi * 2147483648.0;
			bitCount = 32;
			byte[] second76 = NavigationData.Dec2Bin(in value3, in bitCount);
			value3 = ephemeris.ArgumentOfPerigee * NavigationDataINavFNav.Rad2Semi * 2147483648.0;
			bitCount = 32;
			byte[] second77 = NavigationData.Dec2Bin(in value3, in bitCount);
			value3 = ephemeris.MeanMotionCorrection * NavigationDataINavFNav.Rad2Semi * 8796093022208.0;
			bitCount = 16;
			byte[] second78 = NavigationData.Dec2Bin(in value3, in bitCount);
			IEnumerable<byte> second79 = NavigationData.ZeroBits.Take(16);
			IEnumerable<byte> second80 = NavigationData.ZeroBits.Take(16);
			IEnumerable<byte> second81 = NavigationData.ZeroBits.Take(16);
			IEnumerable<byte> second82 = NavigationData.ZeroBits.Take(16);
			value3 = (double)t0e * (1.0 / 60.0);
			bitCount = 14;
			byte[] second83 = NavigationData.Dec2Bin(in value3, in bitCount);
			bitCount = pageTime.GalileoNavicWeek;
			int bitCount2 = 12;
			byte[] second84 = NavigationData.Dec2Bin(in bitCount, in bitCount2);
			bitCount = pageTime.GalileoNavicSecondOfWeek;
			bitCount2 = 20;
			byte[] second85 = NavigationData.Dec2Bin(in bitCount, in bitCount2);
			IEnumerable<byte> second86 = NavigationData.ZeroAndOneBits.Take(8);
			return first.Concat(iodNavBits4).Concat(second76).Concat(second77)
				.Concat(second78)
				.Concat(second79)
				.Concat(second80)
				.Concat(second81)
		...
		case PageId.Page4:
		{
			...
			IEnumerable<byte> second22 = NavigationData.ZeroBits.Take(32);
			IEnumerable<byte> second23 = NavigationData.ZeroBits.Take(24);
			...
			int value5 = gnssTime2.GalileoNavicDayOfWeek + 1;
			int value6 = gnssTime2.GalileoNavicWeek;
			int value7 = leapSecond2.Seconds;
			bitCount = 8;
			byte[] second24 = NavigationData.Dec2Bin(in value4, in bitCount);
			double value3 = (double)(start.GalileoNavicDayOfWeek * 86400) * 0.00027777777777777778;
			bitCount = 8;
			byte[] second25 = NavigationData.Dec2Bin(in value3, in bitCount);
			bitCount = start.GalileoNavicWeek;
			int bitCount2 = 8;
			byte[] second26 = NavigationData.Dec2Bin(in bitCount, in bitCount2);
			bitCount = 8;
			byte[] second27 = NavigationData.Dec2Bin(in value6, in bitCount);
			bitCount = 3;
			byte[] second28 = NavigationData.Dec2Bin(in value5, in bitCount);
			bitCount = 8;
			byte[] second29 = NavigationData.Dec2Bin(in value7, in bitCount);
			IEnumerable<byte> second30 = NavigationData.ZeroBits.Take(16);
			IEnumerable<byte> second31 = NavigationData.ZeroBits.Take(12);
			...
			int value8 = gnssTime3.GalileoNavicWeek;
			value3 = (double)num * 0.00027777777777777778;
			bitCount = 8;
			byte[] second32 = NavigationData.Dec2Bin(in value3, in bitCount);
			bitCount = 6;
			byte[] second33 = NavigationData.Dec2Bin(in value8, in bitCount);
			bitCount = pageTime.GalileoNavicSecondOfWeek;
			bitCount2 = 20;
			byte[] second34 = NavigationData.Dec2Bin(in bitCount, in bitCount2);
			IEnumerable<byte> second35 = NavigationData.ZeroAndOneBits.Take(5);
			return first.Concat(iodNavBits).Concat(second20).Concat(second21)
				.Concat(second22)
				.Concat(second23)
				.Concat(second24)
				.Concat(second25)
		...
		case PageId.Page5:
		{
			...
			int value9 = timeOfAlmanac.GalileoNavicWeek;
			int value10 = galileoNavicSecondOfWeek / 600;
			bitCount = 4;
			byte[] second49 = NavigationData.Dec2Bin(in value10, in bitCount);
			bitCount = 2;
			byte[] second50 = NavigationData.Dec2Bin(in value9, in bitCount);
			...
			IEnumerable<byte> second62;
			if (base.Almanac.GetAlmanac(in firstAlmanacIndex2, in pageTime) is Satellite satellite3)
			{
				bitCount = satellite3.Id;
				int bitCount2 = 6;
				byte[] first3 = NavigationData.Dec2Bin(in bitCount, in bitCount2);
				value3 = satellite3.DeltaSqrtA * 512.0;
				bitCount = 13;
				...
				bitCount = 16;
				byte[] second54 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite3.DeltaInclination * NavigationDataINavFNav.Rad2Semi * 16384.0;
				bitCount = 11;
				byte[] second55 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite3.LongitudeOfAscendingNode * NavigationDataINavFNav.Rad2Semi * 32768.0;
				bitCount = 16;
				byte[] second56 = NavigationData.Dec2Bin(in value3, in bitCount);
				...
				value3 = satellite3.MeanAnomaly * NavigationDataINavFNav.Rad2Semi * 32768.0;
				bitCount = 16;
				byte[] second58 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite3.Af0 * 524288.0;
				bitCount = 16;
				byte[] second59 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite3.Af1 * 274877906944.0;
				bitCount = 13;
				byte[] second60 = NavigationData.Dec2Bin(in value3, in bitCount);
				value = ((!satellite3.IsHealthy) ? 1u : 0u);
				bitCount = 2;
				byte[] second61 = NavigationData.Dec2Bin(in value, in bitCount);
				second62 = first3.Concat(second52).Concat(second53).Concat(second54)
					.Concat(second55)
					.Concat(second56)
					.Concat(second57)
					.Concat(second58)
			...
			{
				second62 = NavigationData.ZeroBits.Take(6).Concat(NavigationData.ZeroAndOneBits.Take(125));
			}
			AlmanacBase almanacBase5 = base.Almanac;
			bitCount = firstAlmanacIndex2 + 1;
			IEnumerable<byte> second67;
			if (almanacBase5.GetAlmanac(in bitCount, in pageTime) is Satellite satellite4)
			{
				...
				bitCount = 11;
				byte[] second66 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite4.LongitudeOfAscendingNode * NavigationDataINavFNav.Rad2Semi * 32768.0;
				bitCount = 16;
				byte[] source2 = NavigationData.Dec2Bin(in value3, in bitCount);
				second67 = first4.Concat(second63).Concat(second64).Concat(second65)
					.Concat(second66)
					.Concat(source2.Take(4));
			}
			else
			{
				second67 = NavigationData.ZeroBits.Take(6).Concat(NavigationData.ZeroAndOneBits.Take(55));
			}
			return first.Concat(second49).Concat(second50).Concat(second51)
				.Concat(second62)
				.Concat(second67);
		}
		case PageId.Page6:
		{
			AlmanacBase almanacBase = base.Almanac;
			bitCount = 0;
			int value2 = almanacBase.GetTimeOfAlmanac(in pageTime, in bitCount).GalileoNavicSecondOfWeek / 600;
			bitCount = 4;
			...
			IEnumerable<byte> second7;
			if (almanacBase2.GetAlmanac(in bitCount, in pageTime) is Satellite satellite)
			{
				double value3 = satellite.LongitudeOfAscendingNode * NavigationDataINavFNav.Rad2Semi * 32768.0;
				bitCount = 16;
				byte[] source = NavigationData.Dec2Bin(in value3, in bitCount);
				...
				bitCount = 2;
				second7 = Enumerable.Concat(second: NavigationData.Dec2Bin(in value, in bitCount), first: source.Skip(4).Take(12).Concat(second2)
					.Concat(second3)
					.Concat(second4)
					.Concat(second5));
			}
			...
			{
				second7 = NavigationData.ZeroAndOneBits.Take(70);
			}
			AlmanacBase almanacBase3 = base.Almanac;
			bitCount = firstAlmanacIndex + 2;
			IEnumerable<byte> second18;
			if (almanacBase3.GetAlmanac(in bitCount, in pageTime) is Satellite satellite2)
			{
				bitCount = satellite2.Id;
				int bitCount2 = 6;
				byte[] first2 = NavigationData.Dec2Bin(in bitCount, in bitCount2);
				double value3 = satellite2.DeltaSqrtA * 512.0;
				bitCount = 13;
				byte[] second8 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite2.Eccentricity * 65536.0;
				bitCount = 11;
				byte[] second9 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite2.ArgumentOfPerigee * NavigationDataINavFNav.Rad2Semi * 32768.0;
				bitCount = 16;
				byte[] second10 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite2.DeltaInclination * NavigationDataINavFNav.Rad2Semi * 16384.0;
				bitCount = 11;
				byte[] second11 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite2.LongitudeOfAscendingNode * NavigationDataINavFNav.Rad2Semi * 32768.0;
				bitCount = 16;
				byte[] second12 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite2.RateOfLongitudeOfAscendingNode * NavigationDataINavFNav.Rad2Semi * 8589934592.0;
				bitCount = 11;
				byte[] second13 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite2.MeanAnomaly * NavigationDataINavFNav.Rad2Semi * 32768.0;
				bitCount = 16;
				byte[] second14 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite2.Af0 * 524288.0;
				bitCount = 16;
				byte[] second15 = NavigationData.Dec2Bin(in value3, in bitCount);
				value3 = satellite2.Af1 * 274877906944.0;
				bitCount = 13;
				byte[] second16 = NavigationData.Dec2Bin(in value3, in bitCount);
				value = ((!satellite2.IsHealthy) ? 1u : 0u);
				bitCount = 2;
				byte[] second17 = NavigationData.Dec2Bin(in value, in bitCount);
				second18 = first2.Concat(second8).Concat(second9).Concat(second10)
					.Concat(second11)
					.Concat(second12)
					.Concat(second13)
					.Concat(second14)
			...
			{
				second18 = NavigationData.ZeroBits.Take(6).Concat(NavigationData.ZeroAndOneBits.Take(125));
			}
			IEnumerable<byte> second19 = NavigationData.ZeroAndOneBits.Take(3);
			return first.Concat(second).Concat(second7).Concat(second18)
				.Concat(second19);
		}
		default:
