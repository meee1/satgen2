	...
	internal sealed class ModulationTiered : ModulationBPSK
	{
		...
		public sealed override sbyte[] Modulate()
		{
			...
			else
			{
				...
				uint num9 = (uint)Math.Round((double)(modulationRate / num4) * IntervalLength);
				uint num6 = (uint)(chipOfWeek / num4 % num5);
				uint size2 = num9 * num4;
				pinnedBuffer = ModulationBank.GetPinnedBuffer(in TimeStamp, in size2);
				uint num7 = 0u;
				for (uint num8 = num9; num8 != 0; num8--)
				{
					Array.Copy(array[secondaryCode[num6]], 0L, pinnedBuffer, num7, num4);
					num7 += num4;
					if (++num6 >= num5)
					{
						num6 = 0u;
