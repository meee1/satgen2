	...
	public sealed class LabSat3wOutput : StreamOutput
	{
		...
		public static ChannelPlan GetChannelPlan(IEnumerable<SignalType> signalTypes, Quantization desiredQuantization)
		{
			...
			if (!signals.Any())
			{
				...
				Quantization quantization4 = channel.Quantization;
				bandwidth = channel.SampleRate;
				int dataRate = GetDataRate(in sampleRate, quantization4, in bandwidth);
		...
		{
			foreach (List<FrequencyBand> item3 in partition)
			{
				uint num = uint.MaxValue;
				uint num2 = 0u;
				foreach (FrequencyBand item2 in item3)
				{
					if ((uint)item2 < num)
					{
						num = (uint)item2;
