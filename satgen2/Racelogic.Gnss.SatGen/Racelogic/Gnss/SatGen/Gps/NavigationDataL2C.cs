...
{
	internal sealed class NavigationDataL2C : NavigationDataCNav
	{
		private static readonly NavigationDataInfo dataInfo = Signal.AllSignals.SelectMany((Signal s) => s.NavigationDataInfos).First((NavigationDataInfo ndi) => ndi.NavigationDataType == NavigationDataType.GpsL2C);

		private static readonly SignalType signalType = Signal.AllSignals.First((Signal s) => s.NavigationDataInfos.Any((NavigationDataInfo ndi) => ndi.NavigationDataType == NavigationDataType.GpsL2C)).SignalType;
		...
		private static readonly SyncLock fecLock = new SyncLock("L2C FEC dictionary lock", 10000);
		...
		public NavigationDataL2C(in int satIndex, Almanac almanac, in Range<GnssTime, GnssTimeSpan> interval, IEnumerable<Signal> signals)
		...
		internal static void FinalizeFEC(in GnssTime intervalEnd)
		{
			...
			using (fecLock.Lock())
			{
				...
				{
					RLLogger.GetLogger().LogMessage($"ERROR: No L2C FEC library entry found for the interval ending on GPS week {intervalEnd.GpsWeek} and second {intervalEnd.GpsSecondOfWeekDecimal} (index of data packet (within a week) for the FEC entry: {packetIndex})");
				}
			}
		}
