...
internal sealed class FloatOutput : StreamOutput
{
	...
	internal override int SampleSize => 64;

	internal override int WordLength => 64;

	internal override int SamplesInWord => 1;

	public FloatOutput(string filePath, IEnumerable<SignalType> signalTypes, in int sampleRate)
		: base(filePath)
	{
	...
	{
		return new FloatQuantizer(buffer, in rms);
	}
	...
	public static ChannelPlan GetChannelPlan(IEnumerable<SignalType> signalTypes, in int sampleRate)
	{
		...
		array[0] = new Channel(signals, frequencyBands, in channelFrequency, in actualFrequency, in sampleRate, in sampleRate, Quantization.Float);
		int dataRate = sampleRate << 3;
