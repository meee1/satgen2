...
public class LiveNmeaTrajectory : Trajectory, ILiveTrajectory
{
	...
	private const double MinutesToDegrees = 1.0 / 60.0;
	...
	private Pvt? ReadGgaSample(NmeaData nmea)
	{
		...
		double latitude = (double)gGA.LatitudeMinutes * (1.0 / 60.0);
		double longitude = (double)gGA.LongitudeMinutes * (1.0 / 60.0);
	...
	private (GnssTime Time, double Velocity)? ReadRmcVelocity(NmeaData nmea)
	{
		...
		GnssTime item = rmcMidnight + GnssTimeSpan.FromSeconds(num);
		double item2 = rMC.SpeedOverGroundKnots * (463.0 / 900.0);
		return new(GnssTime, double)?((item, item2));
	}
	...
	private (GnssTime Time, double Velocity)? ReadVtgVelocity(NmeaData nmea)
	{
		...
		GnssTime item = vtgMidnight + GnssTimeSpan.FromSeconds(num);
