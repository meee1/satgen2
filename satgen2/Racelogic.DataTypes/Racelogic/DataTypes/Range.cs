...
namespace Racelogic.DataTypes
{
	...
	public readonly struct Range<T> : IEquatable<Range<T>> where T : struct, IComparable, IFormattable, IConvertible, IComparable<T>, IEquatable<T>
	{
		...
		{
			T value3 = GenericOperator<T, T, T>.Subtract(MinField, extension);
			T value2 = GenericOperator<T, T, T>.Add(MaxField, extension);
			return new Range<T>(value3, value2);
		}

		public Range<T> ExtendBy(T lowerExtension, T upperExtension)
		{
			T value3 = GenericOperator<T, T, T>.Subtract(MinField, lowerExtension);
		...
		public Range<T> ExtendOverlapping(Range<T> range)
		{
			...
			T minField = MinField;
			T value3 = ((minField.CompareTo(range.MinField) > 0) ? range.MinField : MinField);
			minField = MaxField;
			...
			if (!range1.IsEmpty && !range2.IsEmpty)
			{
				...
				if (minField.CompareTo(range2.MaxField) <= 0)
				{
					...
					if (minField.CompareTo(range2.MinField) >= 0)
					{
						...
						T value3 = ((minField.CompareTo(range2.MinField) > 0) ? range1.MinField : range2.MinField);
						minField = range1.MaxField;
		...
		public static Range<T> Merge(Range<T> range1, Range<T> range2)
		{
			...
			T minField = range1.MinField;
			T value3 = ((minField.CompareTo(range2.MinField) > 0) ? range2.MinField : range1.MinField);
			minField = range1.MaxField;
	...
	public readonly struct Range<T, U> : IEquatable<Range<T, U>> where T : struct, IComparable, IFormattable, IComparable<T>, IEquatable<T> where U : struct, IComparable, IFormattable, IComparable<U>, IEquatable<U>
	{
		...
		{
			T value3 = GenericOperator<T, U, T>.Subtract(StartField, extension);
			T value2 = GenericOperator<T, U, T>.Add(EndField, extension);
			return new Range<T, U>(value3, value2);
		}

		public Range<T, U> ExtendBy(U lowerExtension, U upperExtension)
		{
			T value3 = GenericOperator<T, U, T>.Subtract(StartField, lowerExtension);
		...
		public Range<T, U> ExtendOverlapping(Range<T, U> range)
		{
			...
			T startField = StartField;
			T value3 = ((startField.CompareTo(range.StartField) > 0) ? range.StartField : StartField);
			startField = EndField;
			...
			if (!range1.IsEmpty && !range2.IsEmpty)
			{
				...
				if (startField.CompareTo(range2.EndField) <= 0)
				{
					...
					if (startField.CompareTo(range2.StartField) >= 0)
					{
						...
						T value3 = ((startField.CompareTo(range2.StartField) > 0) ? range1.StartField : range2.StartField);
						startField = range1.EndField;
		...
		public static Range<T, U> Merge(Range<T, U> range1, Range<T, U> range2)
		{
			...
			T startField = range1.StartField;
			T value3 = ((startField.CompareTo(range2.StartField) > 0) ? range2.StartField : range1.StartField);
			startField = range1.EndField;
