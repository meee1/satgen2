...
namespace Racelogic.Geodetics
{
	...
	public readonly struct Geodetic
	{
		...
		{
			GeodesicData val = GeodesicForDatum(datum).Direct(Latitude * 57.295779513082316, Longitude * 57.295779513082316, initialHeading * 57.295779513082316, distance);
			return new Geodetic(val.Latitude2* (Math.PI / 180.0), val.Longitude2* (Math.PI / 180.0), Altitude);
		}

		public double SurfaceDistance(in Geodetic destination, Datum datum = null)
        {
            return GeodesicForDatum(datum).Inverse(Latitude * 57.295779513082316, Longitude * 57.295779513082316,
                destination.Latitude * 57.295779513082316, destination.Longitude * 57.295779513082316).Distance;
        }

		public double SurfaceDistance(in Geodetic destination, out double initialHeading, Datum datum = null)
		{
			GeodesicData val = GeodesicForDatum(datum).Inverse(Latitude * 57.295779513082316, Longitude * 57.295779513082316, destination.Latitude * 57.295779513082316, destination.Longitude * 57.295779513082316);
			initialHeading = val.InitialAzimuth* (Math.PI / 180.0);
            return val.Distance;
        }
		...
		public double GetGeocentricRadius(Datum datum = null)
		{
			...
			double num7 = datum.SemiMajorAxis * datum.SemiMajorAxis;
			double num2 = Math.Cos(Latitude);
			double num3 = num7 * num2 * num2;
			double num4 = datum.SemiMinorAxis * datum.SemiMinorAxis;
			double num5 = Math.Sin(Latitude);
			double num6 = num4 * num5 * num5;
			return Math.Sqrt((num7 * num3 + num4 * num6) / (num3 + num6));
		}

		public string ToGGALine(DateTime time)
		{
			double num8 = Math.Abs(Latitude * 57.295779513082316);
			int num2 = (int)num8;
			double num3 = (num8 - (double)num2) * 60.0;
			char c = ((Latitude >= 0.0) ? 'N' : 'S');
			double num9 = Math.Abs(Longitude * 57.295779513082316);
			int num4 = (int)num9;
			double num5 = (num9 - (double)num4) * 60.0;
			char c2 = ((Longitude > 0.0) ? 'E' : 'W');
			string text = $"GPGGA,{time.Hour:00}{time.Minute:00}{time.Second:00}.{time.Millisecond:00},{num2:00}{num3:00.000000000},{c},{num4:000}{num5:00.000000000},{c2},8,12,,{Altitude:0.000000},M,,,,";
			int num6 = 0;
			string text2 = text;
			foreach (int num7 in text2)
			{
				num6 ^= num7;
			}
			return $"${text}*{num6:X2}";
		...
		public string ToCoordinateString(int secondDecimalPlaces = 5)
		{
			...
			{
				decimal num4 = decimal.Round(Math.Abs(206264.80624709635515647335733m * (decimal)radians), secondDecimalPlaces);
				int num2 = (int)(num4 * 0.0166666666666666666666666667m);
				int result;
				int item7 = Math.DivRem(num2, 60, out result);
				double num3 = (double)(num4 - (decimal)(60 * num2));
		...
		{
			return ((5993773 + Latitude.GetHashCode()) * 9973 + Longitude.GetHashCode()) * 9973 + Altitude.GetHashCode();
		}
	}
}
