...
public readonly struct LocalTangentPlane : IEquatable<LocalTangentPlane>
{
	...
	{
		double north = other.North;
		if (north.Equals(North))
		{
			north = other.East;
			if (north.Equals(East))
			{
				north = other.Down;
				return north.Equals(Down);
			}
		}
		return false;
	}
	...
	{
		double north = North;
		int num = (5993773 + north.GetHashCode()) * 9973;
		north = East;
		int num2 = (num + north.GetHashCode()) * 9973;
		north = Down;
		return num2 + north.GetHashCode();
	}
}
