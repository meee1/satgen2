...
public struct LongitudeMinutes : IRacelogicData, IEquatable<IRacelogicData>, IComparable, IComparable<IRacelogicData>, IConvertible
{
	private readonly double _data;

	public LongitudeMinutes(double value)
	{
		_data = value;
	}

	public LongitudeMinutes(string value)
	{
	...
	private static string ToString(IFormatProvider provider, LatLongUnit units, ToStringOptions options, string format, double data)
	{
		...
		case LatLongUnit.DegreesDecimalMinutes:
		{
			...
			{
				arg2 = ((data < 0.0) ? Resources.West : Resources.East);
			}
		...
		default:
			throw new ArgumentOutOfRangeException("units", string.Format(provider, "{0}{1}{2}", "Longitude.ToString()", Environment.NewLine, "value passed in for longitude units is not recognised - " + units));
		}
	}
	...
	{
		return ToString(UnitsGlobal.CurrentCulture, LatLongUnit.Minutes, ToStringOptions.AlwaysSigned, "0000.00000000", 0.0 - data);
	}

	public static string ToVboString(double data, LatLongUnit unit)
	{
		return ToString(UnitsGlobal.CurrentCulture, unit, ToStringOptions.AlwaysSigned, "0000.00000000", 0.0 - data);
	}

	public static LongitudeMinutes Parse(string s)
	{
		return double.Parse(s);
	}

	public static bool TryParse(string s, out LongitudeMinutes result)
	{
		double result2;
	...
	public static implicit operator LongitudeMinutes(double value)
	{
		return new LongitudeMinutes(value);
	}

	public static implicit operator double(LongitudeMinutes value)
	{
		return value._data;
	}
}
